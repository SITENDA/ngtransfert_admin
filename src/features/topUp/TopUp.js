...
import {Typography, Button, Grid, Card, CardActions, CardContent, Divider} from '@mui/material';
...
const TopUp = () => {
    ...
    useEffect(() => {
        if (countryFetched && countryData?.data?.country) {
            ...
            dispatch(setObjectItem({key: "topUpInputs", innerKey: "sourceCurrencyCode", value: countryData?.data?.country?.currency?.currencyCode}))
        }
            ...
            if ((isValidNumber(topUpInputs.sourceAmount) || isValidNumber(topUpInputs.targetAmount)) &&
                ...
                try {
                    ...
                    if (directionOfExchange === "forward") {
                        dispatch(setObjectItem({key: "topUpInputs", innerKey: "targetAmount", value: currencyExchangeData?.targetAmount}))
                    }
                    else {
                        dispatch(setObjectItem({key: "topUpInputs", innerKey: "sourceAmount", value: currencyExchangeData?.sourceAmount}))
                    }
    ...
    }, [countryOfDeposit?.countryId, topUpInputs.sourceCurrencyCode, topUpInputs.targetCurrencyCode, dispatch, doCurrencyExchange, sourceCurrencyCode, topUpInputs.sourceAmount, topUpInputs.targetAmount]);
                                    ...
                                    {validTopUp.validCountryOfTopUpId && validTopUp.validTopUpMethod &&
                                        <>
                                            ...
                                            changeHandler={(e) => {
                                                ...
                                                if (e.target.value === 0) {dispatch(setObjectItem({key: "topUpInputs", innerKey: "targetAmount", value: 0}))}
                                        ...
                                        <AmountInput
                                            ...
                                            changeHandler={(e) => {
                                                ...
                                                if (e.target.value === 0) {dispatch(setObjectItem({key: "topUpInputs", innerKey: "sourceAmount", value: 0}))}
                                                return dispatch(handleValidation({
                                                objectName: "topUp",
                                                eventValue: e.target.value,
                                            ...
                                            handleFocus={() => {
                                                console.log("targetAmount focused");

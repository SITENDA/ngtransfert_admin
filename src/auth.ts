...
import {RoleDTO} from "../types/next-auth";
import {jwtDecode} from "jwt-decode";

const BACKEND_API_BASE_URL = process.env.NEXT_PUBLIC_SPRING_BOOT_API_URL;
...
// This function will attempt to refresh the token using the provided refresh token
    ...
    try {
        const response = await fetch(`${BACKEND_API_BASE_URL}/auth/refresh`, {
            ...
            headers: {
                ...
                // Common practice: send refresh token in Authorization header
                "Authorization": `Bearer ${refreshToken}`
            },
            // Alternatively, if your backend expects it in the body:
        ...
        // Assuming refreshData contains new accessToken and its expiration (exp),
        // and optionally a new refreshToken if your backend rotates them.
        const newAccessToken = refreshData.accessToken;
        const newRefreshToken = refreshData.ParefreshToken || refreshToken; // Use new refresh token if provided, else keep old
        const decodedNewToken: { exp: number } = jwtDecode(newAccessToken); // Decode new token for its expiration
        const newAccessTokenExpires = decodedNewToken.exp; // Expiration in seconds
        ...
        console.error("Error during token refresh API call:", refreshError);
        throw refreshError; // Re-throw to be caught by the jwt callback
    }
}
...
export const { handlers, auth, signIn, signOut } = NextAuth({
    ...
    callbacks: {
        ...
        async jwt({ token, user }) {
            ...
            // 'user' is now correctly typed as per next-auth.d.ts User interface
            if (user) {
                // Copy all properties from `user` to the `token`
                token.id = user.id;
                ...
                // No more 'as any' casts needed thanks to next-auth.d.ts
                ...
                token.ekiddako = user.ekiddako;
            }

            // Check if access token has expired
            const currentTimestamp = Date.now(); // Milliseconds
            // --- FIX FOR TS2362 START ---
            let accessTokenExpiresMs = 0;
            if (token.accessTokenExpires !== undefined && token.accessTokenExpires !== null) {
                accessTokenExpiresMs = token.accessTokenExpires * 1000;
            }
            // --- FIX FOR TS2362 END ---


            if (token.accessTokenExpires !== undefined && token.accessTokenExpires !== null && currentTimestamp > accessTokenExpiresMs) {
                console.warn("JWT Callback: Backend access token has expired. Attempting to refresh.");
                ...
                if (typeof token.refreshToken ==="string" && token.refreshToken.length > 0) {
                    try {
                        ...
                        token.refreshToken = refreshedTokens.refreshToken; // Update refresh token if rotated

                        console.log("JWT Callback: Token successfully refreshed.");
                        return token; // Return the updated token
                    } catch (refreshError) {
                        console.error("JWT Callback: Token refresh failed. Invalidating session.", refreshError);
                        return null; // Force logout if refresh fails
                    }
                } else {
                    console.warn("JWT Callback: Access token expired, but no refresh token available. Invalidating session.");
                    return null; // Force logout if no refresh token
                }
            }

            // If access token is still valid (or successfully refreshed)
            return token;
        },

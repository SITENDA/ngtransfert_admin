...
export default function AddReceiverAccountForm({
                                                   receiverAccount,
                                                   initialBanks,
                                                   clientId,
                                               }: Props) {

    console.log("initialBanks:", initialBanks);

    const searchParams = useSearchParams();

    const hasReceiverAccountId =
        searchParams.has("receiverAccountId");

    const orderedReceiverAccountTypes =
        useOrderedReceiverAccountCategories();
    ...
    const defaultValues: RequestReceiverAccount =
        hasReceiverAccountId && receiverAccount
            ? {
                ...
                receiverAccountId:
                    receiverAccount.receiverAccountId ?? 0,

                receiverAccountName:
                    receiverAccount.receiverAccountName ?? "",

                clientId:
                    receiverAccount.clientId ?? clientId,

                qrCodeUrl:
                    receiverAccount.qrCodeUrl ?? "",

                qrCodeContent:
                    receiverAccount.qrCodeContent ?? "",
                ...
                bankId:
                    receiverAccount.bankId ?? -1,

                limit:
                    receiverAccount.limit ?? 60000,

                currencyId:
                    receiverAccount.currencyId ?? -1,

                limitCurrencyId:
                    receiverAccount.limitCurrencyId ?? -1,

                creationDate:
                    receiverAccount.creationDate ??
                    new Date(),
            }
            : emptyValues;

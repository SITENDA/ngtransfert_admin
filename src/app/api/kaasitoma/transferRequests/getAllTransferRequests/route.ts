// src/app/api/kaasitoma/transferRequests/getAllTransferRequests/route.ts
...
export async function GET(request: NextRequest) {
    ...
    if (!BACKEND_API_BASE_URL) {
        console.error('Proxy API (getAllTransferRequests): BACKEND_API_BASE_URL environment variable is not defined.');
        return NextResponse.json({ message: 'Server configuration error: Backend URL is missing.' }, { status: 500 });
    }

    try {
        console.log('Proxy API (getAllTransferRequests): Received GET request.');
        ...
        try {
            ...
            console.error('Proxy API (getAllTransferRequests): Failed to parse backend response as JSON despite OK status:', jsonParseError);
            console.error('Proxy API (getAllTransferRequests): Offending response text:', responseBodyText);
        ...
        console.error('Proxy API (getAllTransferRequests): Unhandled error during fetch:', error);
